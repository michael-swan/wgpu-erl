...
import Language.C.Analysis
  ( CompTyKind (StructTag),
    ...
    IntType (TyBool, TyChar, TyInt, TyLLong),
...
getName sr =
  case sr of
    ...
    AnonymousRef _ -> "<ANONYMOUS>" -- error "Unexpected anonymous SUERef"
...
data CType
  ...
  | -- | @double@.
    CDouble
  | -- | A type defined elsewhere that is being referenced by name. Bool is true if type is opaque.
    CDefined !Bool !Text
  deriving (Eq, Show, Generic)
...
translateType t =
  case t of
    (TypeDefType t@(TypeDefRef (Ident str _ _) (PtrType (DirectType (TyComp (CompTypeRef (NamedRef (Ident identImpl _ _)) _ _)) _ _) _ _) _) _ _)
      | identImpl == str ++ "Impl" -> CDefined True (Text.pack str) -- Opaque
    (TypeDefType t@(TypeDefRef (Ident str _ _) _ _) _ _) ->
      CDefined False (Text.pack str) -- Transparent
    (PtrType tp _ _) ->
